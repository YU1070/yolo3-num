...
def rotate_about_center(src, angle, scale=1.):
    ...
    tempImg = cv2.warpAffine(src, rot_mat, (int(math.ceil(nw)), int(math.ceil(nh))), flags=cv2.INTER_LANCZOS4,
                             borderValue=(0, 255, 0))
    ...
    res = cv2.resize(tempImg, (int(tempImg.shape[1] * seedx[indexx]), int(tempImg.shape[0] * seedy[indexy])),
                     interpolation=cv2.INTER_CUBIC)  # dsize=（2*width,2*height）
    return res
...
def checkOverlap(rectangles, b):
    for a in rectangles:
        ...
        if (a[2] - a[0] + b[2] - b[0]) < (widthmax - widthmin) and (a[3] - a[1] + b[3] - b[1]) < (
                heightmax - heightmin):
            continue
        else:
            return True
...
def generateImage():
    ...
    with open("./data/dataset/train.txt", "w") as wf:
        for i in range(0, 12000):
            ...
            cv2.imwrite("./data/dataset/train/" + str(i) + ".jpg", overlay)
